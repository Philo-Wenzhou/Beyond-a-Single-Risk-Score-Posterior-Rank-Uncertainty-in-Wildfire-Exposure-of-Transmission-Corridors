...
Laplace and L2 logistic have similar discrimination. Relative to the
deterministic comparator, Bayesian/L2 ranking captures more holdout positives
in the top-ranked tail. Relative to L2 logistic, the Bayesian point prediction
is not materially better; its value is the posterior decision layer. An initial
Stan run reproduced the Laplace point-ranking metrics; full convergence and
posterior predictive diagnostics remain future work.
...
\begin{figure}[!htbp]
  ...
  \caption{Paired segment-cluster bootstrap differences for PR-AUC and Capture@10. Bayesian Laplace minus deterministic Capture@10 is 0.0677 with percentile interval [0.0188, 0.1197], while Bayesian Laplace minus L2 logistic is approximately zero.}
  \label{fig:bootstrap}
\end{figure}
\FloatBarrier
